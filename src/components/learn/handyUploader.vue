<template>
  <v-container fluid>
      ...
      <v-col cols="12" md="12" xs="12">
        <template v-if="badgeCounter">
          <template v-if="documentAttachment.length > 0">
            ...
            >
              <v-btn :color="btnColor" @click="openInputDocumentModal"> {{sLang[lang].insertFile}} </v-btn>
            </v-badge>
          </template>
          <template v-else>
            <v-btn :color="btnColor" @click="openInputDocumentModal"> {{sLang[lang].insertFile}} </v-btn>
                ...
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle v-if="Number((attachment.file.size / 1000).toFixed(1)) < 1024 ">
                    <v-chip
                            ...
                            text-color="white"
                    >
                      {{  Number((attachment.file.size / 1000).toFixed(1)) +'  '+ sLang[lang].size.kb}}
                  ...
                  <v-list-item-subtitle v-if="Number((attachment.file.size / 1000).toFixed(1)) > 1024">
                    <v-chip
                            ...
                            text-color="white"
                    >
                      {{  Number(((attachment.file.size / 1000) / 1024 ).toFixed(1)) +'  '+ sLang[lang].size.mb}}
                      <v-icon right>mdi-harddisk</v-icon>
                    </v-chip>
                  </v-list-item-subtitle>
                </v-list-item-content>
    ...
    <div v-if="attachment.length">                
                  <template v-if="attachment.file.name.split('.').pop().toLowerCase() === 'jpg' 
                               || attachment.file.name.split('.').pop().toLowerCase() === 'jpeg' 
                               || attachment.file.name.split('.').pop().toLowerCase() === 'png' 
                               || attachment.file.name.split('.').pop().toLowerCase() === 'tif' 
              ...
              <v-card-actions style="padding: 0">
                <v-tooltip right>
                  ...
                  <span class="BYekan">{{sLang[lang].delete}}</span>
                </v-tooltip>
                <v-tooltip right>
                  <template v-slot:activator="{ on }">
                  ...
                  <span class="BYekan">{{sLang[lang].edit}}</span>
                </v-tooltip>
          ...
          <v-col cols="12" lg="12" md="12" xs="12">
            <v-simple-table :fixed-header="tableFixedHeader" :height="tableHeight + 'px'">
              <template v-slot:default>
                <thead>
                <tr>
                  <th v-if="tableThumbColumn" class="text-left">{{sLang[lang].table.thumb}}</th>
                  <th class="text-left">{{sLang[lang].table.name}}</th>
                  <th class="text-left">{{sLang[lang].table.size}}</th>
                  <th class="text-left">{{sLang[lang].table.action.action}}</th>
                </tr>
                </thead>
                <tbody>
          ...
          <div v-if="attachment.length > 0">
                  ...
                  <td>
                    <v-tooltip top>
                      ...
                      <!--span>{{sLang[lang].table.action.deleteTooltip}}</span ZML-->
      ...
      <v-card>
       ...
       <v-card-text class="BYekan">
        <template v-if="tempAttachment.length === 0">
          <v-file-input v-if="fileAccept !== ''" multiple :accept="fileAccept" 
                        chip v-model="tempAttachment" :label="sLang[lang].insertNewFile">
          </v-file-input>
          <v-file-input v-else multiple v-model="tempAttachment" :label="sLang[lang].insertNewFile">
          </v-file-input>
        </template>
        <template>
         <v-expansion-panels v-if="addFileTag || addFileDescription || changeFileName">
             ...
             <v-expansion-panel-content>
              <v-row align="center" justify="center">
               <v-col cols="12" lg="9" md="9" xs="12">
                <v-text-field v-if="changeFileName"
                             :label="sLang[lang].fileName"
                              v-model="attachment.name"
                              :prepend-icon="attachment.icon">
                </v-text-field>
                <v-textarea v-if="addFileDescription"
                           :label="sLang[lang].fileDescription" auto-grow
                                row-height="1" v-model="attachment.description">
                </v-textarea>
                <v-autocomplete v-if="addFileTag"
                                v-model="attachment.tags"
                                :items="tags"
                                dense chips
                                :label="sLang[lang].fileTags"  multiple>
                </v-autocomplete>
               </v-col>
          ...
          <v-card-actions>
            <v-btn class="BYekan" :disabled="tempAttachment == null || btnLoader" :loading="btnLoader" :color="btnColor" @click="uploadFieldChange">{{sLang[lang].add}}</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
    ...
    <v-row justify="center">
      <v-dialog v-model="editDocumentDialog" :scrollable="false" persistent width="50%">
       <v-card>
        <v-card-title>
          Update file information. <v-icon @click="editDocumentDialog = false">mdi-close</v-icon>
        </v-card-title>
         ...
         <div v-if="attachment.name">
          <v-row align="center" justify="center">
           <v-col cols="12" lg="9" md="9" xs="12">
             ...
             <v-textarea v-if="addFileDescription"
                        :label="sLang[lang].fileDescription"
                        auto-grow
                        row-height="1"
             ...
             <v-col align="center" cols="12" lg="3" md="3" xs="12">
              <div v-if="attachment.length > 0">
               <template v-if="attachment.name.split('.').pop().toLowerCase() === 'jpg' || attachment.name.split('.').pop().toLowerCase() === 'jpeg' || attachment.name.split('.').pop().toLowerCase()  === 'png' || attachment.name.split('.').pop().toLowerCase() === 'tif' || attachment.name.split('.').pop().toLowerCase() === 'bmp'">
                <v-card>
                 <v-img :src="'data:'+ attachment.format  + ',' + attachment.base64"></v-img>
                </v-card>
               </template>
               <template v-else>
          ...
          <v-card-actions>
           <v-btn class="BYekan" :disabled="tempAttachment === null || btnLoader" :loading="btnLoader" :color="btnColor" @click="editDocument">{{sLang[lang].edit}}</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
    ...
    <!--File Uploader SnackBar Alert End -->



    <!--DELETE Dialog Start--------------------------------------------------------START---->
    <v-row justify="center">
      <v-dialog v-model="deleteDocumentDialog" persistent width="30%">
        <v-card>
          ...
          <v-card-text class="BYekan">
            {{sLang[lang].deleteDialog.message}}
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn class="BYekan" color="green darken-1" text @click="deleteDocumentDialog= false">{{sLang[lang].deleteDialog.cancel}}</v-btn>
            <v-btn class="BYekan" color="primary" :disabled="btnLoader" :loading="btnLoader" @click="deleteDocument">{{sLang[lang].delete}} </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
...
<script>
  ...
  export default {
    props: {
       ...
       */
      cardType: String,
      badgeCounter: {        type: Boolean,        default: true      },
      btnColor: {        type: String,        default: 'info'      },
      /**
      ...
      /**       * set table height       */
      tableHeight: {        type: Number,        default: 200      },
      rtlSupport: {        type: Boolean,        default: false      },
      lang: {        type: String,        default: 'en'      },
      changeFileName: {        type: Boolean,        default: false      },
      addFileDescription: {        type: Boolean,        default: false      },
      /**
       * Add file tags before upload
       */
      addFileTag: {        type: Boolean,        default: false      },
      tags: {        type: Array,      },
      /**       * Change count of columns       */
      cols: {        type: Number,        default: 4      },
      customLang: {        type: Object,        default: null      },
      insertPermission: {        type: Boolean,        default: true      },
        ...
        for(let [index,item] of this.tempAttachment.entries()){
          if(this.documentAttachment.length < this.maxFileCount) {
            if((item.size / 1000).toFixed(1) > this.maxFileSize){
              ...
              this.fileUploaderSnackText= `${this.sLang[this.lang].maxFileSizeAlert} ${Math.round(this.maxFileSize / 1024)} ${this.sLang[this.lang].size.mb}`;
            ...
            this.fileUploaderSnackBarAlertColor= 'red';
            this.fileUploaderSnackText= `${this.sLang[this.lang].maxFileCountAlert} ${this.maxFileCount}`
            this.fileUploaderSnackBarAlert= true;
          }
        }
