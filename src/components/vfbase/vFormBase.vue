            ...
            <slot :name="getTypeItemSlot(obj)" v-bind= "{ obj, index, id }">
                ...
                >
                  ...
                  >
                    ...
                    <template v-for="s in getInjectedScopedSlots(id, obj)" #[s]>
                      <slot :name= "getKeyInjectSlot(obj, s)" 
              ...
              <!-- CHECKBOX | SWITCH -->
                ...
                wschecknpx
                  <!-- component doesn't work with #[s]="slotData" " -->
                  <template v-for="s in getInjectedScopedSlots(id, obj)" #[s]>
                    <slot :name="getKeyInjectSlot(obj, s)" v-bind= "{ id, obj, index }"/>
                  </template>
                </component>
              <!-- END CHECKBOX | SWITCH -->
              ...
              <!-- MASK  -->
                  ...
                  removed v-mask for the moment... (ZML)-->              
                <component
                  :is="mapTypeToComponent(obj.schema.type)"
...
export default {
  ...
  // TOOLTIP 
    getShorthandTooltip(schemaTooltip){
      ...
      return isString(schemaTooltip) ? { bottom:false, label:schemaTooltip} : schemaTooltip
    },
  ...
  // GRID
    ...
    getGridAttributes(obj){ 
      ...
      const colSchema = obj.schema.col || obj.schema.flex || 12

      const colAttr = this.col || this.flex || colDefault 
 //zml - not needed -  console.log('........',obj.key , colSchema, colAttr)   
