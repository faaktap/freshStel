...
<!-- this img is not displayed; it is used to detect failure-to-load of div background image -->
  ...
  <div :style="[style, customStyle]" aria-hidden="true" @click="testSubmitEmit">
  <v-avatar><span v-show="!this.isImage">{{ userInitial }}</span></v-avatar>
  </div>
</template>
...
export default {
  ...
  methods: {
    testSubmitEmit(e1) {
        console.log('passed back',e1)
        this.$emit('clickAvatar',e1)
    },
    initial: getInitials,
